...
def process_dataframe(dataframe):
    datetime_series = pd.to_datetime(dataframe['date'].astype(str), format='%Y%m%d')
...
def mean_absolute_percentage_error_u(y_pred, y_true):
    y_true = y_true.float()
    y_pred = y_pred.float()
    return torch.mean(torch.abs((y_true - y_pred) / y_true)) * 100

def get_test_plot(generator_model, X_test, y_test, y_scaler, index_test, output_dim, title='Test set'):
    # Set output steps
    ...
    plt.legend(("Real price", "Predicted price"), loc="upper left", fontsize=10)
