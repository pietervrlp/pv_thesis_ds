...
from keras.layers import Bidirectional, LSTM, Dense, Flatten, Conv1D, MaxPooling1D, Dropout
...
def Generator(input_dim, output_dim, feature_size, weight_initializer) -> tf.keras.models.Model:    
    ...
    # model.add(LSTM(64, activation='relu', kernel_initializer= weight_initializer, return_sequences=True, 
    #                              dropout=0.2, recurrent_dropout=0.0))
    model.add(LSTM(64, activation='relu', kernel_initializer= weight_initializer, return_sequences=False, 
                                 dropout=0.2, recurrent_dropout=0.0))                                 
    model.add(Flatten())
