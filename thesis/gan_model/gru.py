...
from keras.layers import Bidirectional, LSTM, Dense, Flatten, Conv1D, MaxPooling1D, Dropout, GRU
from keras.layers import BatchNormalization, LeakyReLU, ReLU, TimeDistributed
...
def Generator(input_dim, output_dim, feature_size, weight_initializer) -> tf.keras.models.Model:    
    ...
    model.add(Dense(32, kernel_initializer=weight_initializer, input_shape=(input_dim,feature_size)))
    model.add(LeakyReLU(alpha=0.1))
    # model.add(Dropout(0.2))
    # model.add(GRU(64, activation='relu', kernel_initializer= weight_initializer, return_sequences=True, 
    #                              dropout=0.2, recurrent_dropout=0.0))
    model.add(GRU(64, activation='relu', kernel_initializer= weight_initializer, return_sequences=False, 
                                 dropout=0.2, recurrent_dropout=0.0)) 
    model.add(Flatten())
...
class WGAN(tf.keras.Model):
    def __init__(self, generator, discriminator, n_steps_in, n_steps_out, trial_id):
    ...
    def train(self, X_train, y_train, past_y, epochs):
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % 100 == 0:
                tf.keras.models.save_model(self.generator, './outcome/gen_models/%s_%d_%d_%d.h5' % (self.trial_id,self.n_steps_in,self.n_steps_out,epoch))
